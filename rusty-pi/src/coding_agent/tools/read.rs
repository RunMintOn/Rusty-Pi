...
async fn read_image(path: &Path) -> anyhow::Result<Vec<Content>> {
    ...
    let mime = detect_image_mime_type_from_file(path).await?.unwrap_or("image/jpeg");
...
mod tests {
    ...
    async fn read_with_offset_and_limit() {
        ...
        assert!(
            lines.len() <= 8,
            "Should have at most 5 content lines + continuation (got {})",
            lines.len()
        );
    }
    ...
    async fn read_image_png() {
        ...
        let png_bytes: Vec<u8> = vec![
            ...
            0x08, 0xD7, 0x63, 0x60, 0x60, 0x00, 0x00, 0x00, 0x04, 0x00, 0x01, 0x27, 0x34,
            0x27, // compressed data + CRC
        ...
        match &result.content[0] {
            Content::Image { data, mime_type } => {
                ...
                assert_eq!(
                    decoded.len(),
                    png_bytes.len(),
                    "Decoded image should match original size"
                );
            }
    ...
    async fn read_image_jpeg() {
        ...
        let jpeg_bytes: Vec<u8> = vec![
            ...
            0x01, 0x05, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03,
            0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0xFF, 0xC4, 0x00, 0xB5, 0x10, 0x00, 0x02, 0x01, 0x03, 0x03,
            0x02, 0x04, 0x03, 0x05, 0x05, 0x04, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x11, 0x04,
            0x05, 0x21, 0x12, 0x31, 0x41, 0x06, 0x13, 0x51, 0x61, 0x07, 0x22, 0x71, 0x14, 0x32, 0x81, 0x91, 0xA1, 0x08,
            ...
            0x00, 0x00, 0x3F, 0x00, 0x7B, 0x94, 0x11, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60,
            0x82,
        ];
    ...
    fn try_curly_quote_variant_replaces_apostrophe() {
        ...
        assert!(
            ...
            "Should contain right single quote: {:?}",
            result
        );
    }
    ...
    async fn resolve_read_path_finds_file_via_nfd() {
        ...
        assert!(
            text.contains("nfc content"),
            "Should find file with NFC name: got {}",
            text
        );
    }
}
